...
--launch firefox
    ...
    awful.key({ modkey },            "r",     function ()
      ...
      -- Changing default prompt to dmenu almost the same the command as before but for dmenu
...
client.connect_signal("property::sticky", function(c)
      -- do something
      naughty.notify{ text='the sticky value of '.. c.name.. "is now: ".. tostring(c.sticky) }
       end)
clientkeys = gears.table.join(
        -- Make a client sticky
        awful.key({ modkey, "Shift"}, "s", function (client) 
          --naughty.notify{text=tostring(client.focus.sticky)}
...
for i = 1, 9 do
    ...
    -- -- Sticky toggle tag display
    --     awful.key({ modkey, "Control", "Alt_R" }, "#" .. i + 9,
    --               function ()
    ...
    --                   end
    --               end,
    --               {description = "toggle tag #" .. i.. " sticky", group = "tag"}),
        -- View tag only.
...
client.connect_signal("manage", function (c)
    ...
    -- uncomment the next line to have DWM like master/slave tiling other wise new client will prompt to the left instead of right
...
--awful.tag.gap_single_client = false
-- local net = require("my_modules.internet_systray")
-- net.sys()
--Autostart the wanted applications
awful.spawn.with_shell('picom --fade-in-step=1 --fade-out-step=1 --fade-delta=0')
awful.spawn.with_shell('volumeicon')
--to prevent screen tearing on nvidia card
